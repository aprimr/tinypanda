...
| **`loop`** | Evaluates and loops continuously while the condition remains true. | `loop (count < 5) { ... }` |
| **`for`** | Initiates a structured layout iteration loop containing state step assignments. | `for (`init`; `condition`; `iteration`) { ... }` |
